...
public:
    ...
    bool add_point(const X &x, const Y &y) {
        ...
        if (points_in_hull > 0 && x <= last_x)
        {
            ...
            // printf("points_in_hull = %d --- x = %.2lf --- last_x = %.2lf\n", points_in_hull, x, last_x);
...
size_t make_segmentation(size_t n, size_t epsilon, Fin in, Fout out) {
    ...
    for (size_t i = 1; i < n-1; ++i) {
        ...
        {
            X next_val;
            if ((next_val = std::nextafter(next_p.first, std::numeric_limits<X>::infinity())) < in(i + 1).first)
            {
                next_p.first = next_val;
            }
            else
            {
                continue;
                // next_val = next_p.first + 1;

                // if (next_val < in(i + 1).first)
                //     next_p.first = next_val;
            }
        }
        ...
        // }
        // p = next_p;
        // // printf("p.first = %.2lf --- p.second = %lu\n", p.first, p.second);
        // if (!opt.add_point(p.first, p.second)) {
        //     out(opt.get_segment());
        //     opt.add_point(p.first, p.second);
        //     ++c;
    ...
    if (n >= 2 && in(n-1).first != in(n-2).first)
    {
        p = in(n-1);

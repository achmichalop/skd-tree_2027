...
inline size_t index_size() {
    size_t tree_size = bench::common::get_boost_rtree_statistics(*rtree);

    tree_size += bench::common::recordsInLeaves * sizeof(Point) + bench::common::recordsInInternals * (sizeof(Box) + sizeof(void *));
...
inline Points range_query(Box& box) {
    ...
    auto end = std::chrono::steady_clock::now();
    // std::cout<<"size = " << return_values.size()<<std::endl;
    range_count++;
    range_time += std::chrono::duration_cast<std::chrono::microseconds>(end - start).count();
...
inline Points knn_query(Point& q, unsigned int k) {
    ...
    // std::vector<double> vec_dists(k, 0);
    // for (int i = 0; i < k; i++)
    // {   
    //     vec_dists[i] = bench::common::eu_dist_square(q, return_values[i]); 
    // }

    // std::sort(vec_dists.begin(), vec_dists.end());
    // for (int i = 0; i < k; i++)
    // {   
    //     std::cout << vec_dists[i] << " - ";
    // }
